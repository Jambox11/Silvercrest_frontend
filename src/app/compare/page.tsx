...
import { getCompareIds, removeCompare } from '@/lib/compare';
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16">
        <EmptyState
          ...
          description="Add up to three listings to the compare tray from Discover or a property page."
        />
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <PageHeader
        ...
        subtitle="Side-by-side specs, fractional math, and Stellar account links for up to three properties."
      />
            ...
            {[
              ...
              [
                ...
                (p: (typeof properties)[0]) =>
                  formatPrice(listings.find(l => l.propertyId === p.id)?.price ?? p.price, p.currency),
              ],
              ...
              ['Fractional', (p: (typeof properties)[0]) => (p.fractional ? 'Yes' : 'No')],
              ['Share price', (p: (typeof properties)[0]) => (p.sharePrice ? formatPrice(p.sharePrice, p.currency) : '—')],
              ['Cap rate', (p: (typeof properties)[0]) => (p.capRate != null ? `${p.capRate.toFixed(2)}%` : '—')],
              [
                'Owner',
                (p: (typeof properties)[0]) => p.owner,
              ],
            ].map(([label, getter]) => (
              <tr key={String(label)} className="border-t border-gray-100">
                <td className="p-3 font-medium text-gray-500">{label as string}</td>
                {properties.map(p => {
                  const value = (getter as (p: (typeof properties)[0]) => string)(p);
                  if (label === 'Owner') {
                    return (
                      <td key={p.id} className="p-3">
                        <a
                          href={explorerAccountUrl(network, p.owner)}
                          target="_blank"
